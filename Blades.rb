...
class Blades
    ...
    def self.bladeRepository()
        "#{ENV["HOME"]}/Galaxy/DataHub/Blades/Data/Repository"
    end
    ...
    def self.merge(filepath1, filepath2)
        ...
        if !File.exist?(filepath1) then
            raise "> incorrect Blades::merge(filepath1, filepath2) request with filepath1: #{filepath1}, filepath2: #{filepath2}, filepath1 does not exist"
        end

        if !File.exist?(filepath2) then
            raise "> incorrect Blades::merge(filepath1, filepath2) request with filepath1: #{filepath1}, filepath2: #{filepath2}, filepath2 does not exist"
        end

        if (filepath1 == filepath2) then
            raise "> incorrect Blades::merge(filepath1, filepath2) request with filepath1: #{filepath1}, filepath2: #{filepath2}, identical locations"
        end

        uuid1 = Blades::getMandatoryAttribute1(filepath1, "uuid")
        uuid2 = Blades::getMandatoryAttribute1(filepath2, "uuid")

        if uuid1 != uuid2 then
            raise "> incorrect Blades::merge(filepath1, filepath2) request with filepath1: #{filepath1}, filepath2: #{filepath2}, uuids are different, uuid1: #{uuid1}, uuids2: #{uuid2}"
        end
    ...
    def self.putDatablob1(filepath, datablob)
        ...
        datablob_check = Blades::getDatablobOrNull1(filepath, nhash)
        ...
        if File.size(filepath) > 1024*1024*1024 then # 1Gb Mb
            ...
            if nextuuid then
                Blades::putDatablob2(nextuuid, datablob) # if that one if too full, we will carrying on recursively until the end
                return
            else
                currentuuid = Blades::getMandatoryAttribute1(filepath, "uuid")
                nextuuid = SecureRandom.uuid
                Blades::init("NxPure", nextuuid)
                Blades::putDatablob2(nextuuid, datablob)
                Blades::setAttribute2(nextuuid, "previous", currentuuid)
                Blades::setAttribute2(currentuuid, "next", nextuuid)
                return
            end
        end

        puts "Blades::putDatablob1(filepath: #{filepath}, nhash: #{nhash})".green
        db = SQLite3::Database.new(filepath)
        db.busy_timeout = 117
        db.busy_handler { |count| true }
        db.results_as_hash = true
...
class BladeUtils
    ...
    # BladeUtils::itemOrNull1(filepath)
